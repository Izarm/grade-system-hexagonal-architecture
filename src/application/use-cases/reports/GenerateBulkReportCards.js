// src/application/use-cases/reports/GenerateBulkReportCards.js
...
class GenerateBulkReportCards {
    ...
    async generateSingleReportCard(studentId, academicYearId) {
        ...
        // Organizar notas por período (mismo código que en GenerateStudentReportCard)
        const periods = {};
        grades.forEach(g => {
        ...
        return new Promise((resolve, reject) => {
            ...
            doc.on('error', reject);
            // (Aquí va todo el código de generación del PDF, igual que en GenerateStudentReportCard)
    ...
    buildPDF(doc, student, enrollment, sortedPeriods, periods, academicYearId) {
        ...
        for (const periodName of sortedPeriods) {
            ...
            for (const subject of periods[periodName].subjects) {
                ...
                if (subject.average !== null) {
                    if (subject.average >= 6) doc.fillColor('#10b981');
                    else if (subject.average >= 3) doc.fillColor('#f59e0b');
        ...
        const overall = count > 0 ? (sum / count).toFixed(2) : '-';
        doc.fontSize(12).font('Helvetica-Bold').text(`Promedio General: ${overall}`, { align: 'right' });
        doc.moveDown(2);
